...
export class ProjectsApi {
	...
	async getAll(options = {}) {
		...
		try {
			const response = await this.apiClients.data.graphql(
				PROJECTS_QUERIES.GET_ALL_PROJECTS,
				{ first, page }
			);
			...
			return {
				data: response.data?.projects?.data || [],
				paginatorInfo: response.data?.projects?.paginatorInfo || {
					currentPage: page,
					lastPage: 1,
					total: 0,
					perPage: first,
	...
	async getByAgent(agentId) {
		...
		try {
			const response = await this.apiClients.data.graphql(
				PROJECTS_QUERIES.GET_PROJECTS_BY_AGENT,
	...
	async getById(projectId) {
		...
		try {
			const response = await this.apiClients.data.graphql(
				PROJECTS_QUERIES.GET_PROJECT_BY_ID,
				{ id: String(projectId) }
			);
	...
	async search(searchTerm, options = {}) {
		try {
			...
			const filteredData = result.data.filter(project => 
